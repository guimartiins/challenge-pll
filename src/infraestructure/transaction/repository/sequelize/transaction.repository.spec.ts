...
describe('transaction repository unit tests', () => {
  ...
  it('should transfer money between accounts', async () => {
    ...
    const transaction = new Transaction('123', payer.id, receiver.id, 500)
    ...
    expect(transactionModel?.payer_id).toEqual(transaction.payerId)
    expect(transactionModel?.receiver_id).toEqual(transaction.receiverId)
  })
})
